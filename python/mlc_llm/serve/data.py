...
class ImageData(Data):
    ...
    @staticmethod
    def phi3v_from_url(url: str, config: Dict) -> "ImageData":  # pylint: disable=too-many-locals, unused-argument
        """Get the image from the given URL, process and return the image tensor as TVM NDArray."""
        ...
        global_image = torch.nn.functional.interpolate(image_tensor.unsqueeze(0).float(), \
                size=(336, 336), mode='bicubic',).to(image_tensor.dtype)

        # [(3, h, w)], where h, w is multiple of 336
        h = image_tensor.size(1)
        w = image_tensor.size(2)
        hd_images_reshape = image_tensor.reshape(1, 3, h//336, 336, w//336, 336)\
