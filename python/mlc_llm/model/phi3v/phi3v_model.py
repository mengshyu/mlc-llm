...
#ref https://huggingface.co/microsoft/Phi-3-vision-128k-instruct/blob/main/image_embedding_phi3_v.py#L26
CLIPVISION_DEFAULT_CONFIG = {
    ...
    "patch_size": 14,
    "projection_dim": 768,
    "vocab_size": None  # This key was not present in the original string, so I added it with a placeholder value.
}
...
class Phi3VConfig(ConfigBase):  # pylint: disable=too-many-instance-attributes
    ...
    vision_config: CLIPVisionConfig = CLIPVisionConfig.from_dict(CLIPVISION_DEFAULT_CONFIG)
...
class Phi3VForCausalLM(nn.Module):
    ...
    def image_embed(self, input_ids: Tensor, pixel_values: Tensor, image_sizes:Tensor) -> Tensor:
        pixel_values = pixel_values.astype(self.dtype)
        return self.vision_embed_tokens(input_ids, pixel_values, image_sizes) #input_ids?
    ...
    def get_default_spec(self):
        mod_spec = {
            ...
            "image_embed": {
                "input_ids": nn.spec.Tensor([1, 1932], "int32",),
                "pixel_values": nn.spec.Tensor([17, 3, self.config.vision_config.image_size, self.config.vision_config.image_size], "float32",),
                "image_sizes": nn.spec.Tensor([2], "int32"),
                "$": {
                    "param_mode": "packed",
                    "effect_mode": "none",
                },
